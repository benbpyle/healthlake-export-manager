...
export default class CdcStateMachineConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: StateMachineConstructProps) {
    ...
    const file = fs.readFileSync(path.resolve(__dirname, "../asl/cdc.json"));

    const logGroup = new LogGroup(this, "CloudwatchLogs", {
      logGroupName: "/aws/vendedlogs/states/healthlake-cdc-export",
      removalPolicy: RemovalPolicy.DESTROY,
      retention: RetentionDays.ONE_DAY,
    });

    const busArn = Fn.importValue("main-mda-bus-arn");
    ...
    const primaryStoreKeyArn = Fn.importValue("account-data-kmskey");
    const kmsPolicy = new PolicyDocument({
      statements: [
        new PolicyStatement({
          ...
          resources: [
            props.dataKey.keyArn,
            props.queueKey.keyArn,
            primaryStoreKeyArn,
          ],
          effect: Effect.ALLOW,
        }),
